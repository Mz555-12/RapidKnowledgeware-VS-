...
namespace OllamaFramework.Rag.Impl
{
...
public class RagService : IRagService
{
    ...
    public async Task<int> IndexDocumentAsync(string filePath, string[] chunkSeparators, bool clearExisting = false)
    {
        ...
        Debug.WriteLine($"[RagService.IndexDocumentAsync] 开始索引: {filePath}，分隔符=[{string.Join(", ", chunkSeparators)}]");
        ...
        Debug.WriteLine($"[RagService.IndexDocumentAsync] 分块完成，共 {chunks.Count} 个块，开始生成嵌入向量...");

        var embeddings = await _embeddingService.GenerateEmbeddingsAsync(chunks);

        Debug.WriteLine($"[RagService.IndexDocumentAsync] 嵌入向量生成完成，共 {embeddings.Count} 个向量");
